...
select year(now()),month(now()),select day(now());

select concat(year(now()),'년', month(now()),'월', select day(now()),'일');

select name, birthday from jusorok;
select name, substr(birthday, 1, 10) from jusorok;

-- 시간(hour()) / qns(minute()) / ch(second())
...
select date_format(birhday, '%Y-%m-%d %W') from jusorok; --%w : 요일을 숫자로 (월요일 : 1)
select date_format(birhday, '%Y-%m-%d %w') from jusorok;

select date_format(now(), '%h: %i : %s'); -- am pm 12시간제
select date_format(now(), '%H: %i : %s'); -- 24시간제
select date_format(now(), '%1: %i : %s'); --시간이 : 3(한자리
select date_format(now(), '%r');  -- Am/Pm 표시한다. 12시간제 (시간 : 03 PM)
select date_format(now(), '%Y-%c-%e');  -- '월'과 '일'의 표현을 한(두)자리로 표시.
select date_format(birthday, '%Y-%c-%e') from jusorok;  -- '월'과 '일'의 표현을 한(두)자리로 표시.
...
-- 날짜 연산(date_add(기존날짜, 계산수치값)) - interval 명령어와 함께 사용...
select now();                                   --오늘날짜
...
--날찌 차이 계산하기(to_days())
select to_days(now()) - to_days('2021-4-1'); -- (오늘 날짜 - 2021년 4월1일)
